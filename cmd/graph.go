...
// Class the type of the binary search tree
type Class generic.Type
...
func (g *AppGraph) AddNode(n *Node) {
	g.lock.Lock()
...
func (g *AppGraph) AddEdge(n1, n2 *Node) {
	g.lock.Lock()
	if g.edges == nil {
...
func (g *AppGraph) Traverse(f func(*Node)) {
	g.lock.RLock()
	q := NodeQueue{}
	q.New()
	n := g.nodes[0]
